...
const QUESTIONS = [
  ...
  },
  {
    ...
    options: [
      { value: 'b1', label: 'Moins de 500 €' },
      { value: 'b2', label: '500 – 1000 €' },
      { value: 'b3', label: '1000 – 2000 €' },
      { value: 'b4', label: 'Plus de 2000 €' }
    ]
  },
  {
    key: 'noise',
...
function scoreProduct(p) {
  ...
  const budgetRanges = { b1: [0, 500], b2: [500, 1000], b3: [1000, 2000], b4: [2000, 999999] };
...
document.getElementById('navCtaBtn').addEventListener('click', () => {
  document.getElementById('homeView').style.display !== 'none' ? null : null;
  document.getElementById('startQuizBtn').click();
});
const seeAllBtnEl = document.getElementById('seeAllBtn');
if (seeAllBtnEl) {
  seeAllBtnEl.addEventListener('click', () => {
    document.getElementById('startQuizBtn').scrollIntoView({ behavior: 'smooth' });
  });
}

/* ============ CATÉGORIES ============ */
const CATEGORIES = [
  { icon: '🛞', label: 'Clims portatives', hint: 'Mobiles, sans installation', filter: p => p.type === 'portatif' },
  { icon: '🌙', label: 'Clims silencieuses', hint: 'Pour dormir sur vos deux oreilles', filter: p => p.decibels <= 22 },
  { icon: '🍃', label: 'Clims économiques', hint: 'Le meilleur rapport qualité/prix', filter: p => p.tier === 'low_cost' },
  { icon: '📏', label: 'Petites surfaces', hint: 'Idéal studio et petites pièces', filter: p => p.surface_max <= 20 },
  { icon: '🏠', label: 'Grandes surfaces', hint: 'Puissance maximale pour grands espaces', filter: p => p.surface_max >= 28 },
  { icon: '⭐', label: 'Meilleur Q/P', hint: 'Les mieux notés du comparatif', filter: p => p.clim_score >= 85 }
];
...
function renderCategories() {
  ...
  CATEGORIES.forEach(cat => {
    const count = CLIM_DATA.filter(cat.filter).length;
    const card = document.createElement('div');
    card.className = 'category-card';
    card.innerHTML = `
      <span class="cat-icon">${cat.icon}</span>
      <b>${cat.label}</b>
      <small>${cat.hint} · ${count} modèles</small>
    `;
...
/* ============ SÉLECTION DU MOMENT (dark) ============ */
function renderSelectionDark() {
  const wrap = document.getElementById('selectionScroll');
  if (!wrap) return;
  const top5 = [...CLIM_DATA].sort((a, b) => b.clim_score - a.clim_score).slice(0, 6);
  const badgeLabels = ['Meilleur choix', 'Ultra silencieux', 'Meilleur rapport Q/P', 'Haute performance', 'Économique', 'Fiable'];
  const tints = ['#639922', '#185FA5', '#EF9F27', '#639922', '#185FA5', '#EF9F27'];
  ...
  top5.forEach((p, idx) => {
    ...
    card.innerHTML = `
      <div class="sel-image">
        <span class="sel-badge">${badgeLabels[idx]}</span>
        ${acIllustration(tints[idx])}
      </div>
      <div class="sel-body">
        <p class="sel-brand">${p.brand}</p>
        <span class="sel-name">${p.name}</span>
        <div class="sel-price-row">
          <span class="sel-price">${p.price} €</span>
          <span class="sel-score">${p.clim_score}/100</span>
        </div>
        <a class="sel-buy" href="${p.affiliate_url}" target="_blank" rel="nofollow noopener">Voir sur Amazon →</a>
      </div>
    `;
    wrap.appendChild(card);
  });
