...
const RANK_LABELS = {
  qp: ['🥇 Meilleur rapport Q/P', '🥈 Alternative', '🥉 Alternative'],
...
function buildPool() {
  return CLIM_DATA.map(p => ({ p, s: scoreProduct(p) }))
    .sort((a, b) => b.s - a.s)
    .slice(0, 10);
}
...
function sortPoolByTab(key) {
  ...
  switch (key) {
    ...
    default: return list.slice(0, 3); // déjà trié par score personnalisé (qp)
  }
}
